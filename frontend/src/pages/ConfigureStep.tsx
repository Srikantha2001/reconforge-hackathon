...
export function ConfigureStep({ config, onConfigChange, onApproved }: ConfigureStepProps) {
  ...
  const canAuthor = seedInfo?.exists && !loading
  ...
  const author = async () => {
    ...
    try {
      const cfg = await api.authorConfig({
        ...
        actor_id: actingAs,
        columns_a: seedInfo.ledger_columns,
        columns_b: seedInfo.statement_columns,
      })
      onConfigChange(cfg)
      setDirty(false)
    } catch (e) {
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" gutterBottom>
            1. Describe the reconciliation
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            The LLM authors a schema-validated config from this — it never matches
            transactions itself. {seedInfo?.exists === false && 'Generate seed data first via the API to enable authoring.'}
          </Typography>
          <TextField
            multiline
            minRows={3}
        ...
        <Card>
          <CardContent>
            <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
              <Typography variant="h6">2. Review — {config.recon_name}</Typography>
